...
def build_orders(
    ...
    list[tuple[object, ...]],  # order_rows（最终状态行，供对照/验收）
    list[tuple[object, ...]],  # detail_rows
    list[tuple[object, ...]],  # payment_rows
    list[tuple[object, ...]],  # refund_rows
    dict[int, datetime],  # order_id -> pay_time（交易阶段 2 的 UPDATE 时间）
    dict[int, datetime],  # order_id -> refund_time（交易阶段 3 的 UPDATE 时间）
]:
    order_rows = []
    detail_rows = []
    payment_rows = []
    ...
    # 交易阶段时间映射：循环内已生成的随机值，直接记录（不得再调 rng，保证可复现）
    pay_time_map: dict[int, datetime] = {}
    refund_time_map: dict[int, datetime] = {}
    ...
    for idx in range(1, order_count + 1):
        ...
        if paid:
            ...
            pay_time_map[order_id] = pay_time
            payment_rows.append(
                (
                    order_id,
                    out_trade_no,
        ...
        if refunded:
            ...
            refund_time_map[order_id] = refund_time
            refund_rows.append(
                (
                    order_id,
                    user_id,
    ...
    return order_rows, detail_rows, payment_rows, refund_rows, pay_time_map, refund_time_map


def make_initial_order(row: tuple[object, ...]) -> tuple[object, ...]:
    """下单时刻的订单行：全量初始态 UNPAID（未支付、无物流）。

    差异字段（与最终状态行对照）：
      order_status -> UNPAID, payment_way -> NULL, operate_time/update_time -> create_time,
      process_status -> CREATED, tracking_no -> NULL
    """
    cols = list(row)
    cols[4] = "UNPAID"    # order_status
    cols[6] = None        # payment_way
    cols[12] = row[11]   # operate_time = create_time（index 12，非 13）
    cols[14] = "CREATED"  # process_status（index 14）
    cols[15] = None       # tracking_no（index 15）
    cols[22] = row[11]   # update_time = create_time
    return tuple(cols)
...
def main() -> None:
    ...
    orders, details, payments, refunds, pay_time_map, refund_time_map = build_orders(
        rng, dt, args.users, args.orders, user_pool
    )
    paid_rows = [r for r in orders if r[4] == "PAID"]
    refunded_rows = [r for r in orders if r[4] == "REFUNDED"]
    initial_orders = [make_initial_order(r) for r in orders]

    # V2 交易生命周期：单 run 分事务段（同 seed 三段可对账）。
    # 段1 下单全量 UNPAID -> 段2 UPDATE PAID + payment_info -> 段3 UPDATE REFUNDED + refund
    lines = [
    ...
    # ---- 交易阶段 2: 支付（UPDATE order_info -> PAID + 插入 payment_info）----
    if paid_rows:
        lines.append("-- 阶段2: 支付")
        ...
        for row in paid_rows:
            oid = row[0]
            pay_ts = pay_time_map[oid]
            lines.append(
                f"UPDATE order_info SET order_status = 'PAID', payment_way = {sql_quote(row[6])}, "
                f"process_status = 'FINISHED', tracking_no = {sql_quote(row[15])}, "
                f"operate_time = {sql_quote(pay_ts)}, update_time = {sql_quote(pay_ts)} "
                f"WHERE id = {oid};"
            )
        lines.append("")
        write_insert(
    ...
    # ---- 交易阶段 3: 退款（UPDATE order_info -> REFUNDED + 插入 refund）----
    if refunded_rows:
        lines.append("-- 阶段3: 退款")
        lines.append("START TRANSACTION;")
        lines.append("")
        for row in refunded_rows:
            oid = row[0]
            refund_ts = refund_time_map[oid]
            lines.append(
                f"UPDATE order_info SET order_status = 'REFUNDED', "
                f"operate_time = {sql_quote(refund_ts)}, update_time = {sql_quote(refund_ts)} "
                f"WHERE id = {oid};"
            )
        lines.append("")
        write_insert(
    ...
    lines.append(
        f"-- rows: users={len(users_to_upsert)} orders={len(orders)} "
        f"details={len(details)} payments={len(payments)} refunds={len(refunds)} "
        f"(paid_updates={len(paid_rows)} refund_updates={len(refunded_rows)})"
    )
